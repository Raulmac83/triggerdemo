...
public class AuditCustomerHandler : IAuditTriggerHandler
{
    private readonly INotificationWriter _writer;
    private readonly ITriggerMethodSelector _selector;

    public AuditCustomerHandler(INotificationWriter writer, ITriggerMethodSelector selector)
    {
        _writer = writer;
        _selector = selector;
    }
    ...
    public bool ShouldHandle(AuditTriggerEvent evt)
        => evt.EntityName.EndsWith(".Customer", StringComparison.Ordinal) || evt.EntityName == "Customer";
